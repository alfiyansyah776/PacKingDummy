...
class EditProfileActivity : AppCompatActivity() {
    ...
    private fun updateData(){
        ...
        if (binding.edtusername.text.toString() != username) {
            databaseReference?.child(currentUser?.uid!!)?.child("Username")?.setValue(binding.edtusername.text.toString())
        }

        if (binding.edtname.text.toString() == "" || binding.edtname.text.toString() != name){
            databaseReference?.child(currentUser?.uid!!)?.child("Name")?.setValue(binding.edtname.text.toString())
        }

        if (binding.edtemail.text.toString() != name){
            databaseReference?.child(currentUser?.uid!!)?.child("Email")?.setValue(binding.edtemail.text.toString())
        }

        if (binding.edtaddress.text.toString() == "" || binding.edtaddress.text.toString() != name){
            databaseReference?.child(currentUser?.uid!!)?.child("Address")?.setValue(binding.edtaddress.text.toString())
        }
